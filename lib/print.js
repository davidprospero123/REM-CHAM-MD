...
export default async function (m, conn = { user: {} }) {
  ...
  console.log(
    `
${chalk.hex("#FFC0CB").bold("╭─────────────────────────────────────────────────────────────────────────────╮")}
🤖 ${chalk.magentaBright("%s")} ⏱️ ${chalk.black(chalk.bgGreen("%s"))} 📂 ${chalk.black(chalk.bgGreen("%s"))} ${chalk.magenta("%s [%s %sB]")} 
...
`.trim(),
    me +
      " ~" +
      conn.user.name +
      `${conn.user.jid == global.conn.user.jid ? "" : " (Sub Bot)"}`,
