...
let handler = async (m, { conn, usedPrefix, command }) => {
	...
	if (!mime) throw `¿Dónde está la imagen que quieres convertir en caricatura?`;
	...
	try {
		Cartoon(img).then(async (response) => {
			if (response.message == "éxito") {
				...
				m.reply(
					"Disculpe amigo, la imagen no muestra una cara, por favor envíe una foto en la que la cara esté expuesta y visible.."
				);
			}
		});
	} catch {
...
handler.tags = ["AI"];
handler.command = ["tocartoon",'animalo'];
