...
${usedPrefix + command} 1000`.trim()
    ...
    let time = users.lastcf + 90000
        if (new Date - users.lastcf < 90000) throw `Puedes volver a jugar a la pelea de gallos en ${msToTime(time - new Date())}`
        if (amount < 100) throw `🟥 *No puedes apostar oro por menos de 100*`
        if (users.credit < amount) throw `🟥 *No tienes suficiente dinero para esta apuesta.*\n*Actualmente solo tienes ${credit} de gold.*`
        if (users.chicken < 1) {
        throw `🟥 *No tienes pollos para apostar* \nUsa este comando ${usedPrefix}comprarpollo`
    }
    ...
    let status = `Tu pollo murio :c 🪦`

      if (botScore < playerScore) {
        users.credit += amount * 1
        status = `Tu pequeña gallina ganó la pelea y te hizo 🪙 ${amount * 2} gold Mas rico 🐥`
      } else {
        users.credit -= amount * 1
        users.chicken -= 1
...
handler.command = ['cock-fight', 'cf','peleadepollo','combatirpollo']
...
function number(x = 0) {
    x = parseInt(x)
    return !isNaN(x) && typeof x == 'numero'
}