let handler = async (m, { conn, args, text, usedPrefix, command }) => {
	let tee = `✳️ Introduce un texto breve\n\n📌 Ejemplo  : *${usedPrefix + command}* CURI`
	let too = `✳️ Separe el texto con un *+* \n\n📌 Ejemplo : \n*${usedPrefix + command}* CURI *+* BOT`
	...
	switch (type) {
	  case 'gfx1':
		...
		conn.sendFile(m.chat, chut, 'logo.png', `✅ Resultado`, m);
		m.react(done);
		break;
	  ...
	  case 'gfx2': 
		...
		conn.sendFile(m.chat, loda, 'logo.png', `✅ Resultado`, m);
	  ...
	  case 'gfx3':
		...
		conn.sendFile(m.chat, cp, 'logo.png', `✅ Resultado`, m);
		m.react(done);
		break;
	...
	conn.sendFile(m.chat, apiUrl, 'logo.png', `✅ Resultado`, m)
	m.react(done)
	break 
	case 'gfx7': 
	if (!text) throw too;
	...
	conn.sendFile(m.chat, oda, 'logo.png', `✅ Resultado`, m);
	...
	conn.sendFile(m.chat, od, 'logo.png', `✅ Resultado`, m);
	...
	conn.sendFile(m.chat, rr, 'logo.png', `✅ Resultado`, m);
	m.react(done);
	break;
