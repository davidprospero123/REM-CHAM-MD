...
async function handler(m, { conn, args, usedPrefix, command }) {
    ...
    const item = items.filter(v => v in user && typeof user[v] == 'numero')
    let lol = `✳️ Forma correcta de usar el comando
...
`.trim()
    ...
    if (!(who in global.db.data.users)) return m.reply(`✳️ El usuario no esta en mi base de datos :c`)
    if (user[type] * 1 < count) return m.reply(`✳️  *${type}*  insuficiente para transferir`)
    let confirm = `
    Estas seguro que quieres transferir *₹${count}* para *@${(who || '').replace(/@s\.whatsapp\.net/g, '')}* ? 
...
`.trim()
    ...
    confirmation[m.sender] = {
        ...
        type,
        count,
        timeout: setTimeout(() => (m.reply('⏳ Teimpo perdido'), delete confirmation[m.sender]), 60 * 1000)
    }
}
...
handler.before = async m => {
    ...
    if (/yes?/g.test(m.text.toLowerCase())) {
        ...
        user[type] -= count * 1
        _user[type] += count * 1
        if (previous > user[type] * 1 && _previous < _user[type] * 1) m.reply(`Transferencia Correcta ✅ \n\n*₹${count}* Fue transferido a @${(to || '').replace(/@s\.whatsapp\.net/g, '')}`, null, { mentions: [to] })
        else {
            user[type] = previous
...
handler.command = ['payxp', 'transfer', 'give','transferir']
