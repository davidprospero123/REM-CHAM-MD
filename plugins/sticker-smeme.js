...
let handler = async (m, {
    ...
    if (/video/g.test(mime)) {
        if ((quotedMessage.msg || quotedMessage).seconds > 11) return m.reply('Maximo 10 segundos!')
    }
    
    if (!/webp|image|video|gif|viewOnce/g.test(mime)) return m.reply(`Responder a los medios con el comando.\n\n${usedPrefix + command} <${topText ? topText : 'top text'}>|<${bottomText ? bottomText : 'bottom text'}>`)
