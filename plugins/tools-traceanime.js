...
let handler = async (m, { conn }) => {
  try {
    ...
    if (!result || result.error || result.result.length === 0) {
      throw "*Error: no se pudo rastrear el anime.*";
    }
    ...
    message += `*Similar:* ${similarity.toFixed(2)}%\n`;
    message += `*Time:* ${formatDuration(from * 1000)} - ${formatDuration(to * 1000)}\n`;

    if (episode) {
      message += `*Episodios:* ${episode}\n`;
    }

    console.log("Anime Informacion:", {
      animeTitle,
